...
jQuery.fn.quickpaginate = function( settings ) {
	...
	var setNav = function()
	{
		...
		//上一页和下一页文字
		var pagerNav = $('<a class="'+settings.prev+'" href="#" style="border:1px solid #333;margin-right:5px;color:#fff;background:#666;display:inline-block;height:22px;width:70px;line-height:22px;font-size:13px;">Previous</a><a class="'+settings.next+'" href="#" style="border:1px solid #333;margin-left:5px;color:#fff;background:#666;display:inline-block;height:22px;width:70px;line-height:22px;font-size:13px;">Next</a>');

		jQuery(settings.pager).append( pagerNav );

		if ( settings.showcounter )
		{
			var counter = '<span class="'+settings.counter+'"><span class="'+settings.pagenumber+'" style="font-size:12px;"></span>/<span class="'+settings.totalnumber+'"  style="font-size:12px;"></span></span>';
